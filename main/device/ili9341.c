...
void ili9341_flush( lv_display_t *disp, const lv_area_t *area, uint8_t * px_map)
{
    ...
    for (int i=0; i< lv_area_get_width(area) * lv_area_get_height(area) * 2; i++) {
        printf("%x ", px_map[i]);
    }
...
void test_flush( lv_display_t *disp, const lv_area_t *area, uint8_t * px_map)
{
    ...
    lcd_cmd(0x2C, false);
    lcd_data(px_map, lv_area_get_width(area) * lv_area_get_height(area) * 2);
